...
class RatingAggregatorSpec
  ...
  "RatingAggregator" should {
    ...
    "add ratings and compute aggregates correctly" in {
      ...
      val ratingAggregator: ActorRef[RatingMessage] =
        spawn(RatingAggregator(), "ratingAggregator")
      ...
      // Add ratings with proper timing
      ratings.foreach { rating =>
        ratingAggregator ! AddRating(showName, rating)
      }

      val probe = createTestProbe[Option[Aggregates]]()
      ...
      val expectedAggregates = Some(
        Aggregates(expectedAverage, expectedReach, expectedEngagementScore)
      )
    ...
    "return None for a show with no ratings" in {
      // Create the RatingAggregator actor
      val ratingAggregator: ActorRef[RatingMessage] =
        spawn(RatingAggregator(), "ratingAggregatorEmpty")
